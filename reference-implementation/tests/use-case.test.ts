...
function makeFakes() {
  const courseTable: Record<string, Course> = {
    c1: { id: "c1", code: "CS101", title: "Intro", creditValue: 3, courseType: "CORE" },
    c2: { id: "c2", code: "MA101", title: "Calc", creditValue: 2, courseType: "CORE" },
  };
  const resultTable: ResultRecord[] = [
    { id: "r1", studentId: "s1", courseId: "c1", semesterId: "sem1", componentScores: [], finalScore: 85, isLocked: false },
    { id: "r2", studentId: "s1", courseId: "c2", semesterId: "sem1", componentScores: [], finalScore: 30, isLocked: false },
  ];
  const updates: Record<string, Partial<ResultRecord>> = {};
  const auditEntries: unknown[] = [];

  const results: ResultRepository = {
    async findById(id) { return resultTable.find((r) => r.id === id) ?? null; },
    async findAll() { return resultTable; },
    async create(e) { const x = { ...e, id: "new" } as ResultRecord; resultTable.push(x); return x; },
    async update(id, patch) { updates[id] = patch; const r = resultTable.find((x) => x.id === id)!; Object.assign(r, patch); return r; },
    async softDelete() {},
    async findByStudentAndSemester(sid, sem) { return resultTable.filter((r) => r.studentId === sid && r.semesterId === sem); },
    async findByStudent(sid) { return resultTable.filter((r) => r.studentId === sid); },
    async existsFor() { return false; },
  };
  const courses: CourseRepository = {
    async findById(id) { return courseTable[id] ?? null; },
    async findAll() { return Object.values(courseTable); },
    async create(e) { const x = { ...e, id: "new" } as Course; return x; },
    async update(_id, _p) { return Object.values(courseTable)[0]!; },
    async softDelete() {},
    async findByCode(code) { return Object.values(courseTable).find((c) => c.code === code) ?? null; },
    async findByProgramme() { return Object.values(courseTable); },
  };
  const audit: AuditLogPort = {
    async record(e) { auditEntries.push(e); },
  };
...
describe("ProcessSemesterResults use-case", () => {
  it("computes GPA, persists grades, and writes an audit entry", async () => {
    ...
    const summary = await uc.execute({ studentId: "s1", semesterId: "sem1", scale });
