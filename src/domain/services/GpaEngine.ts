...
export class GpaEngine {
  ...
  processSemester(results: CourseResult[]): GpaSummary {
    const courses: ProcessedCourseResult[] = results.map((r) => {
      if (r.creditValue <= 0) {
        throw new Error(
          `Course ${r.courseCode} has a non-positive credit value.`,
        );
      }
  ...
  computeCumulative(summaries: GpaSummary[]): {
    ...
    const creditsAttempted = summaries.reduce(
      (s, x) => s + x.creditsAttempted,
      0,
    );
    const creditsEarned = summaries.reduce((s, x) => s + x.creditsEarned, 0);
    const totalQuality = summaries.reduce(
      (s, x) => s + x.totalQualityPoints,
      0,
    );
    const cgpa =
      creditsAttempted === 0
