...
export function makeCore(overrides: Partial<CoreApi> = {}): CoreApi {
  ...
  const base: CoreApi = {
    ...
    previewMatricule: async () => ({ matricule: "" }),
    readmitStudent: async () => ({}) as never,
    regenerateMatricule: async () => ({ matricule: "" }),
    findDuplicateCandidates: async () => [],
    mergeStudents: async () => ({ ok: true as const }),
    bulkRegenerateMatricules: async () => ({ regenerated: 0, skipped: [] }),
    ...
    unlockResult: async () => {},
    saveCourseResults: async () => ({ saved: 0, skipped: 0, errors: [] }),
    listCourseRoster: async () => [],
    parseWorkbook: async () => [],
